...
def ale_plot(model, train_set, features, filename, bins=10, monte_carlo=False, predictor=None, features_classes=None, **kwargs):
	...
	plt.tight_layout()
	plt.show()
	if not os.path.isdir("output/ale"):
		os.mkdir("output/ale")
	if ".png" not in filename:
		filename += ".png"
	plt.savefig("./output/ale/{}".format(filename))
